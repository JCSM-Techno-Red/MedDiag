...
def exportar_historico(historico: List[Diagnostico]) -> bool:
    ...
    try:
        filepath = filedialog.asksaveasfilename(
            defaultextension=".txt",
            filetypes=[("Text files", "*.txt"), ("All files", "*.*")],
            initialfile="historico_diagnosticos.txt",
            initialdir=ConfigManager.get_path("exports")
        )
...
def exportar_paciente(paciente: Paciente) -> bool:
    ...
    try:
        ...
        lines = [
            ...
            f"Estado Civil: {paciente.estado_civil}",
            f"Tipo Sanguíneo: {paciente.tipo_sanguineo}",
            "",
            "CONTATO:",
            "-" * 40,
            ...
            f"WhatsApp: {paciente.whatsapp}",
            "",
            "ENDEREÇO:",
            "-" * 40,
            f"CEP: {paciente.cep}",
            f"Endereço: {paciente.endereco}, {paciente.numero}",
            f"Complemento: {paciente.complemento}",
            f"Bairro: {paciente.bairro}",
            f"Cidade/UF: {paciente.cidade}/{paciente.estado}",
            f"País: {paciente.pais}",
            "",
            "CONTATO DE EMERGÊNCIA:",
            "-" * 40,
            f"Nome: {paciente.contato_emergencia_nome}",
            f"Telefone: {paciente.contato_emergencia_telefone}",
            f"Parentesco: {paciente.contato_emergencia_parentesco}",
            "",
            "INFORMAÇÕES MÉDICAS:",
            "-" * 40
        ]
        ...
        if paciente.medicamentos_uso:
            ...
            lines.append("Medicamentos: Nenhum registrado")
        ...
        if paciente.cirurgias_previas:
            lines.append("Cirurgias prévias:")
            for cirurgia in paciente.cirurgias_previas:
                lines.append(f"  • {cirurgia}")
        else:
            lines.append("Cirurgias prévias: Nenhuma registrada")
